...
export default function LabelForm({initFormData, ...props}) {
    ...
    useEffect(() => {
        ...
        api.get(URL_MAP.GET_LABEL_PREQUISITE)
            .then((resp) => {
            ...
            .catch((err) => {
                ...
                if (err.response) {
                    ...
                    setFormError(
                        'Failed to fetch pre-requisites. Not able to send the request. Contact administrator.'
                    )
                } else {
                    setFormError(
                        'Failed to fetch pre-requisites. Some error occurred. Contact administrator.'
                    )
                }
            })
            .then(() => {
    ...
    const onSave = () => {
        ...
        if (isFormValid) {
            ...
            if (editMode) {
                tempResponse = api.patch(url, formData)
            } else {
                tempResponse = api.post(url, formData)
            }
            tempResponse
                .then((resp) => {
            ...
            <Box display='flex'>
                ...
                {opLoading && (
                    <>
                        ...
                        />
                        <Typography style={{alignSelf: 'center'}}>&nbsp;Loading...</Typography>
                    </>
                )}
            </Box>
            <Form>
