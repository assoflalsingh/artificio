...
export default class ImageAnnotation extends React.Component {
	...
	fetchThumbnails() {
		...
		this.props.api.post(URL_MAP.GET_THUMBNAILS, {'data_lists': data_lists})
			.then((res)=>{
			...
			if(error.response) {
				this.setState({ajaxMessage: {error: true, text: error.response.data.message}})
			} else {
				console.error(error);
			}
		}).then(()=>{
	...
	render() {
		return(
			<>
				<Backdrop style={{zIndex: 2000}} open={Boolean(this.state.processMsg)}>
					<CircularProgress color="inherit" />
					<Typography style={{marginLeft: '0.25rem'}} variant='h5'>{this.state.processMsg}</Typography>
				</Backdrop>
