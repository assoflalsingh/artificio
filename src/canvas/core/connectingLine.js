import {
  LabelContainerId,
  LabelId,
} from "../../components/ImageAnnotation/label/LabelsContainer";
...
export class ConnectingLine {
  ...
  getConnectingLineEndPoint(pointer) {
    ...
    const stage = {
      x:
        this.canvasManager.annotationLayer.x() *
          this.canvasManager.stage.scale().x +
        this.canvasManager.stage.x(),
      y:
        this.canvasManager.annotationLayer.y() *
          this.canvasManager.stage.scale().y +
        this.canvasManager.stage.y(),
    };
  ...
  isLabelInView() {
    ...
    const labelElementBoundingRect = document
      .getElementById(`${LabelId}-${selectedAnnotation.id}`)
      .getBoundingClientRect();
    return (
      labelElementBoundingRect.y >= boundingRect.y &&
      labelElementBoundingRect.y <= boundingRect.y + boundingRect.height
    );
  }
  ...
  initialize() {
    ...
    const labelElement =
      selectedAnnotation &&
      document.getElementById(`${LabelId}-${selectedAnnotation.id}`);
    if (selectedAnnotation && labelElement && this.isLabelInView()) {
      ...
      const pointer = this.getConnectingLineEndPoint({
