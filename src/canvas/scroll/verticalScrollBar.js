...
export class VerticalScrollBar {
  ...
  initialize() {
    this.scrollbar = new Konva.Rect({
      ...
      dragBoundFunc: (pos) => {
        ...
        pos.y = Math.max(
          Math.min(pos.y, this.height - this.scrollbar.height()),
          0
        );
        return pos;
      },
  ...
  dragMove() {
    ...
    const allowedStageHeightBound =
      this.containerStage.height() - this.containerStage.height() * this.scale;
    this.onDragMove({
      x: this.containerStageStartPosition.x,
      y:
        this.containerStageStartPosition.y -
        diff * ratio * Math.abs(allowedStageHeightBound / this.height),
    });
  }
  ...
  reposition(scale) {
    ...
    if (scale === 1) {
      ...
      this.layer.show();
      const allowedStageHeightBound =
        this.containerStage.height() - this.containerStage.height() * scale;
